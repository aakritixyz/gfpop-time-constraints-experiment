...
Omega::Omega(Graph graph)
{
  ...
  q = graph.nb_edges();
  library(devtools)
load_all("path/to/gfpop")  # loads your modified package
# create a tiny dataset
x <- rnorm(10)
# run Omega function (or wrapper exposed via R)
result <- gfpop(x)
print(result)
...
Omega::~Omega()
{
  if(LP_ts != nullptr)
  {
    for(unsigned int i = 0; i <= n; i++) { delete [] LP_ts[i]; }
    delete [] LP_ts;
    LP_ts = nullptr;
  }
  delete [] LP_edges;
...
void Omega::initialize_LP_ts(unsigned int n)
{
  ...
  for(unsigned char j = 0; j < p; j++)
  {
    ...
    {
      if((m_graph.getEdge(k).getConstraint() == "node") &&
         (m_graph.getEdge(k).getState1() == j))
      {
...
void Omega::backtracking()
{
  ...
  if(endState.empty())
  {
    for(unsigned int j = 1; j < p; j++)
    {
      ...
      if(malsp_temp[0] < malsp[0])
      {
        CurrentState = j;
        malsp[0] = malsp_temp[0];
      }
    }
  }
  else
  {
    for(unsigned int j = 0; j < endState.size(); j++)
    {
      LP_ts[n][endState[j]].get_min_argmin_label_state_position_ListPiece(malsp_temp);
      if(malsp_temp[0] < malsp[0])
      {
        CurrentState = endState[j];
        malsp[0] = malsp_temp[0];
      }
    }
  }
  ...
  while(malsp[2] > 0)
  {
    ...
    correction = (decay != 1) ? std::pow(decay, parameters.back() - malsp[2] + 1) : 1;
    
    constrainedInterval = m_graph.buildInterval(malsp[1]*correction, malsp[3], CurrentState, out);
    
    CurrentState = malsp[3];
    CurrentChgpt = malsp[2];
    
    LP_ts[(int) malsp[2]][(int) malsp[3]].get_min_argmin_label_state_position_onePiece(
        malsp, (int) malsp[4], constrainedInterval, out, boolForced
    );
    
    CurrentGlobalCost -= m_graph.findBeta(malsp[3], CurrentState);
